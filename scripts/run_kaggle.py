...
def write_frontend_dotenv(base_path: str, *, split_proxy: bool) -> Path:
    ...
    Always overwrites local loopback URLs — never leave
    ``http://127.0.0.1:8000`` in the client bundle for Kaggle.
    """
    ...
    lines = [
        ...
        "# Browser must use relative /proxy/8000 (or gateway /api/v1) — never loopback.",
    ]
    for key in keys:
        if key in env and env[key] is not None:
            ...
            if "127.0.0.1" in val or "localhost" in val.lower():
                raise RuntimeError(
                    ...
                    "(Kaggle browsers cannot reach the notebook's localhost)."
                )
...
def frontend_dotenv_is_kaggle_safe(*, split_proxy: bool) -> bool:
    """True when frontend/.env.local is safe for a Kaggle browser (no loopback)."""
    path = FRONTEND / ".env.local"
    if not path.exists():
        return False
    try:
...
def assert_client_bundle_has_no_loopback_api() -> None:
    """Fail the Kaggle launch if the built client still embeds loopback API URLs."""
    ...
    for root in roots:
        ...
        for path in root.rglob("*"):
            if not path.is_file():
                continue
            if path.suffix not in {".js", ".html", ".rsc"}:
                continue
            try:
            ...
            for needle in needles:
                if needle in data:
                    hits.append(f"{path.relative_to(FRONTEND)} contains {needle.decode()}")
                    break
            if len(hits) >= 5:
                break
        if len(hits) >= 5:
    ...
    if hits:
        raise RuntimeError(
            "Frontend build embeds loopback API URLs (Kaggle browsers cannot use them). "
            "Rebuild via scripts/run_kaggle.py after writing proxy .env.local. Hits:\n  - "
            + "\n  - ".join(hits)
        )
...
def verify_frontend_routes(*, base_path: str = "") -> dict[str, int]:
    """Hit Next.js app routes on :3000 (with optional gateway basePath)."""
    prefix = (base_path or "").rstrip("/")
    results: dict[str, int] = {}
    for route in FRONTEND_APP_ROUTES:
        url = f"http://127.0.0.1:3000{prefix}{route}"
        try:
            import urllib.request
            ...
            with urllib.request.urlopen(req, timeout=8) as resp:
                ...
                if route == "/":
                    ...
                    if missing:
                        _log(f"Home UI missing markers: {missing}")
                        results[route] = 0
                        continue
        except Exception as exc:
            code = 0
            if hasattr(exc, "code") and isinstance(exc.code, int):
                code = int(exc.code)
            _log(f"Route probe {url} → {code or 'ERR'} ({exc})")
            results[route] = int(code) if code else 0
            continue
        results[route] = int(code) if code else 0
        _log(f"Route {route} → HTTP {results[route]}")
    return results


def verify_api_health() -> dict[str, int]:
    """Probe FastAPI health + OpenAPI on :8000."""
    out: dict[str, int] = {}
    for path in ("/api/v1/health", "/docs", "/api/v1/openapi.json"):
        url = f"http://127.0.0.1:8000{path}"
        try:
            import urllib.request

            with urllib.request.urlopen(url, timeout=8) as resp:
                out[path] = int(resp.status)
        except Exception as exc:
            code = getattr(exc, "code", 0) if hasattr(exc, "code") else 0
            out[path] = int(code) if isinstance(code, int) else 0
            _log(f"API probe {url} → {out[path] or 'ERR'} ({exc})")
    return out
...
def main() -> int:
    ...
    if deploy.get("is_kaggle") and not frontend_dotenv_is_kaggle_safe(
        split_proxy=split_proxy
    ):
        _log("Kaggle frontend env unsafe — forcing dotenv write + rebuild")
        need_build = True
    ...
    if need_build:
        ...
        if deploy.get("is_kaggle") or split_proxy:
            try:
                assert_client_bundle_has_no_loopback_api()
                _log("Client bundle check OK — no loopback API URLs embedded")
            except RuntimeError as exc:
                _log(f"ERROR: {exc}")
                return 1
        # After rebuild, never reuse a stale Next process.
        if frontend_up:
            _log("Rebuilt frontend — restarting Next.js to pick up new env/bundle")
            stop_port(3000, "frontend")
            frontend_up = False
            next_proc = None
    ...
    try:
        ...
        # Local route + API contract probes (UI frozen; wiring only).
        route_codes = verify_frontend_routes(
            base_path=base_path if not split_proxy else ""
        )
        api_codes = verify_api_health()
        studio_ok = route_codes.get("/studio/custom-garment") in (200, 307, 308)
        studio_redir_ok = route_codes.get("/studio") in (200, 307, 308)
        health_ok = api_codes.get("/api/v1/health") == 200
        if not (studio_ok and studio_redir_ok and health_ok):
            _log(
                "ERROR: Local route/API probe failed — "
                f"routes={route_codes} api={api_codes}"
            )
            print_banner(
                health_code=health_code,
                root_code=root_code,
